...
(function() {
    ...
    function validateEditUsername() {
        var el = document.getElementById('editUsername');
        if (!el) return true;
        var rawU = el.value;
        if (rawU === '') { clearErrorMessage('editUsername'); return true; }

        var firstChar = rawU.charAt(0);
        if (/\s/.test(firstChar)) { showErrorMessage('editUsername', 'Spaces are not allowed in username'); return false; }
        if (firstChar === '_') { showErrorMessage('editUsername', 'Must not start with a special character.'); return false; }
        if (/[^A-Za-z0-9_]/.test(firstChar)) { showErrorMessage('editUsername', 'Must not start with a special character.'); return false; }
        if (/[0-9]/.test(firstChar)) { showErrorMessage('editUsername', 'Must not start with a number.'); return false; }
        if (/[A-Z]/.test(firstChar)) { showErrorMessage('editUsername', 'Capital letters are not allowed'); return false; }

        var idxUndEarly = rawU.indexOf('_');
        if (idxUndEarly !== -1) {
            var suffixEarly = rawU.slice(idxUndEarly + 1);
            if (suffixEarly && /[^0-9]/.test(suffixEarly)) {
                showErrorMessage('editUsername', 'Only numbers are allowed after the underscore.');
                return false;
            }
        }

        if (/\s/.test(rawU)) { showErrorMessage('editUsername', 'Spaces are not allowed in username'); return false; }
        if (/[A-Z]/.test(rawU)) { showErrorMessage('editUsername', 'Capital letters are not allowed'); return false; }
        if ((rawU.match(/_/g) || []).length > 1) { showErrorMessage('editUsername', 'Only one underscore "_" is allowed'); return false; }

        var idxUnd = rawU.indexOf('_');
        if (idxUnd === -1) {
            if (/[^a-z0-9_]/.test(rawU)) { showErrorMessage('editUsername', 'Underscore "_" is the only allowed special character'); return false; }
            if (/^[a-z]/.test(rawU) && rawU.length < 5) {
                showErrorMessage('editUsername', 'Username format must be ex. xxxxx_12345');
                return false;
            }
            var letters = (rawU.match(/^[a-z]+/) || [''])[0].length;
            if (letters > 10) { showErrorMessage('editUsername', 'Maximum of 10 letters only'); return false; }
            if (letters < 5) { showErrorMessage('editUsername', 'Letters must be at least 5 characters long.'); return false; }
            if (/^[a-z]{5,}$/.test(rawU)) { showErrorMessage('editUsername', 'An underscore "_" is required after the first word.'); return false; }
            if (/^[a-z]{5,}[0-9]+$/.test(rawU)) { showErrorMessage('editUsername', 'Underscore is missing. Format must be: xxxxx_1234.'); return false; }
        } else {
            var prefix = rawU.slice(0, idxUnd);
            var suffix = rawU.slice(idxUnd + 1);
            if (/[A-Z]/.test(prefix)) { showErrorMessage('editUsername', 'Capital letters are not allowed'); return false; }
            if (/[^a-z0-9]/.test(prefix)) { showErrorMessage('editUsername', 'Underscore "_" is the only allowed special character'); return false; }
            if (/\d/.test(prefix)) { showErrorMessage('editUsername', 'Underscore "_" must be between letters and numbers (e.g. xxxxx_1234).'); return false; }
            if (!/^[a-z]+$/.test(prefix) || prefix.length < 5) { showErrorMessage('editUsername', 'Letters must be at least 5 characters long.'); return false; }
            if (prefix.length > 10) { showErrorMessage('editUsername', 'Maximum of 10 letters only'); return false; }
            if (suffix === '') { showErrorMessage('editUsername', 'Numbers must come after the underscore.'); return false; }
            if (!/^\d+$/.test(suffix)) { showErrorMessage('editUsername', 'Only numbers are allowed after the underscore.'); return false; }
            if (suffix.length > 6) { showErrorMessage('editUsername', 'Maximum of 6 numbers only'); return false; }
            if (/\s/.test(rawU)) { showErrorMessage('editUsername', 'Spaces are not allowed in username'); return false; }
            if (/[^a-z0-9_]/.test(rawU)) { showErrorMessage('editUsername', 'Underscore "_" is the only allowed special character'); return false; }
        }
        ...
        clearErrorMessage('editUsername');
        return true;
    }
    ...
    function handleEditUsernameInput() {
        var el = document.getElementById('editUsername');
        if (!el) return;
        clearTimeout(editUsernameTimeoutId);
        var rawU = el.value;
        if (rawU === '') { clearErrorMessage('editUsername'); return; }
        if (!validateEditUsername()) return;
        editUsernameTimeoutId = setTimeout(function() {
            checkEditUsernameExists(el.value);
        }, 500);
    }

    function checkEditEmailExists(emailValue) {
        var el = document.getElementById('editEmail');
        if (!el) return;
        if (emailValue === '' || emailValue.trim() === '') {
            clearErrorMessage('editEmail');
            return;
        }
        if (emailValue === editOriginalEmail) { clearErrorMessage('editEmail'); return; }
            ...
            if (xhr.readyState == 4) {
                ...
                if (xhr.status == 200) {
                    try {
                        var response = JSON.parse(xhr.responseText);
                        if (response.exists === true) {
                            showErrorMessage('editEmail', 'Email already exists. Please use a different email.');
                        } else {
                            clearErrorMessage('editEmail');
                        }
                    } catch (e) {}
    ...
    function checkEditUsernameExists(usernameValue) {
        var el = document.getElementById('editUsername');
        if (!el) return;
        if (usernameValue === '' || usernameValue.trim() === '') {
            clearErrorMessage('editUsername');
        ...
        if (usernameValue === editOriginalUsername) { clearErrorMessage('editUsername'); return; }
        el.setAttribute('data-validating', 'true');
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '../../server/check_username.php', true);
        xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            ...
            if (xhr.readyState == 4) {
                ...
                if (xhr.status == 200) {
                    try {
                        var response = JSON.parse(xhr.responseText);
                        if (response.exists === true) {
                            showErrorMessage('editUsername', 'Username already exists. Please choose a different username.');
                        } else {
                            clearErrorMessage('editUsername');
                        }
                    } catch (e) {}
    ...
    function clearEditErrors() {
        ['editFirstName', 'editMiddleName', 'editLastName', 'editExtensionName',
         'editEmail', 'editUsername', 'editStreet', 'editBarangay',
         'editCity', 'editProvince', 'editCountry', 'editZipcode'].forEach(function(id) {
            clearErrorMessage(id);
        });
        clearTimeout(editEmailTimeoutId);
        clearTimeout(editUsernameTimeoutId);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (usersTableBody) {
            ...
            usersTableBody.addEventListener('click', function(e) {
                ...
                editOriginalEmail = user.email || '';
                editOriginalUsername = user.username || '';
                clearEditErrors();
        ...
        var editNameFields = ['editFirstName', 'editMiddleName', 'editLastName', 'editExtensionName'];
        editNameFields.forEach(function(id) {
        ...
        if (editEmailEl) {
            editEmailEl.addEventListener('input', function() {
                clearTimeout(editEmailTimeoutId);
                if (!validateEmail('editEmail')) return;
                var val = editEmailEl.value;
                if (val.trim()) {
                    editEmailTimeoutId = setTimeout(function() {
                        checkEditEmailExists(val);
                    }, 500);
                }
            });
            editEmailEl.addEventListener('blur', function() {
                clearTimeout(editEmailTimeoutId);
                if (!validateEmail('editEmail')) return;
                if (editEmailEl.value.trim()) checkEditEmailExists(editEmailEl.value);
            });
        }

        var editUsernameEl = document.getElementById('editUsername');
        if (editUsernameEl) {
            editUsernameEl.addEventListener('input', handleEditUsernameInput);
            editUsernameEl.addEventListener('blur', function() {
                clearTimeout(editUsernameTimeoutId);
                validateEditUsername();
                if (editUsernameEl.value.trim()) checkEditUsernameExists(editUsernameEl.value);
            });
        }
        ...
        Object.keys(editAddressMap).forEach(function(profileId) {
            var el = document.getElementById(profileId);
            if (el) {
                el.addEventListener('input', function() { validateEditAddressField(profileId, editAddressMap[profileId]); });
                el.addEventListener('blur', function() { validateEditAddressField(profileId, editAddressMap[profileId]); });
            }
        });

        var editZipEl = document.getElementById('editZipcode');
        if (editZipEl) {
            editZipEl.addEventListener('input', function() { validateZip4('editZipcode'); });
            editZipEl.addEventListener('blur', function() { validateZip4('editZipcode'); });
        }
