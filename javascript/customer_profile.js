(function() {
    ...
    var profile = typeof customerProfile !== 'undefined' ? customerProfile : null;
    ...
    function submitProfile(e) {
        ...
        var xhr = new XMLHttpRequest();
        xhr.open('POST', '../server/customer_update_profile.php', true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    try {
                        ...
                        if (res.success) {
                            document.getElementById('successModalMessage').textContent = res.message || 'Profile updated successfully!';
    ...
    function clearAllRequiredErrors() {
        var form = document.getElementById('customerProfileForm');
        if (!form) return;
        var errEls = form.querySelectorAll('[id$="-error"]');
